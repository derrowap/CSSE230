...
//		content.add(this.sp, BorderLayout.CENTER);
	...
	class Panel extends JPanel {
		...
		public void paintComponent(Graphics comp) {
			...
			if (b.root != null) {
				AATree.BinaryNode root = b.root;
				Node n = new Node(this.getWidth() / 2 - 12.5, 25,
						(Integer) root.getElement(), root.getLevel());
				n.drawOn(comp2D);
		...
		public void drawRec(Node n, AATree.BinaryNode par,
			...
			if (par.getLeftChild() != null) {
				Line2D l1 = new Line2D.Double(x+12.5, y +25, x - tx + 12.5, y
						+ ty);
				comp2D.draw(l1);
				AATree.BinaryNode apar = par.getLeftChild();
				n.translate(x - TRANSX, y + TRANSY, (Integer) apar.getElement(), Color.WHITE, apar.getLevel());//(apar.getColor()==AATree.Color.RED)?Color.RED:Color.WHITE);
				n.drawOn(comp2D);
			...
			if (par.getRightChild() != null) {
				Line2D l1 = new Line2D.Double(x + 12.5, y +25, x + tx+12.5, y
						+ ty);
				comp2D.draw(l1);
				AATree.BinaryNode bpar = par.getRightChild();
				n.translate(x + TRANSX, y + TRANSY, (Integer) bpar.getElement(), Color.WHITE, bpar.getLevel());//(bpar.getColor()==AATree.Color.RED)?Color.RED:Color.WHITE);
				n.drawOn(comp2D);
		...
		class Node extends JComponent {
			...
			public Node(double x, double y, int v, int l) {
			...
			public void drawOn(Graphics2D g2) {
				...
				g2.draw(outline);
				g2.drawString("" + this.value + "    " + this.level, (float) (this.CenterX + 9.5),
						(float) (this.CenterY + 16.5));
			}

			public void translate(double x, double y, int v, Color c, int l) {
				this.CenterX = x;
				this.CenterY = y;
				this.value = v;
				this.color = c;
